...
FreeListNode find_node(uint32_t sz){
    ...
    while(temp->flink != NULL){
        if (temp->flink->size-sz>16){
            out=(void*)temp->flink;
            uint32_t oldSize=temp->flink->size;
            out->size=sz;
            FreeListNode newNode=(void*)temp->flink+sz;
            newNode->flink=temp->flink->flink;
            newNode->size=oldSize-sz;
            temp->flink=newNode;
            return out;
        }
        else if (temp->flink->size>sz){
            out=temp->flink;
